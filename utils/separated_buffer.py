...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        # 如果act_shape本身不是元组 (例如对于Discrete空间，它是个整数), 将其转为元组
        if not isinstance(act_shape, tuple):
            act_shape = (act_shape,)

        # 拼接元组来创建最终的形状
        final_action_shape = (self.episode_length, self.n_rollout_threads) + act_shape

        self.actions = np.zeros(final_action_shape, dtype=np.float32)
        self.action_log_probs = np.zeros(final_action_shape, dtype=np.float32)
